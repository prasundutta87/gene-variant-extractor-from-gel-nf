...
normalise_chrX_qc_metrics <- function(df) {
  if (!"MEDIAN_DP_XX" %in% names(df)) return(df)   # XX/XY columns exist only on chrX tables
  ...
  df %>%
    ...
    # which would also remove gnomADg_AF_*_XX / _XY that we keep on purpose.
    select(

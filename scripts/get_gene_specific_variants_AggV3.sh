...
first_vcf_line=$(cat VCF_annotations_${ROW_INDEX}.bed |head -n 1| cut -f13| sed 's|^s3://|filesystems/|')
...
cat VCF_genotypes_${ROW_INDEX}.bed| while read line;do
	variant_vcf=$(echo "$line" | cut -f13| sed 's|^s3://|filesystems/|')
...
cat VCF_siteQC_${ROW_INDEX}.bed| while read line;do
	siteQC_vcf=$(echo "$line" | cut -f13| sed 's|^s3://|filesystems/|')
...
cat VCF_annotations_${ROW_INDEX}.bed| while read line;do
    annotation_vcf=$(echo "$line" | cut -f13| sed 's|^s3://|filesystems/|')
